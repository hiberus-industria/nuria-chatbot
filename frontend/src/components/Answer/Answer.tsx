...
export const Answer = ({ answer, onCitationClicked, onExectResultClicked }: Props) => {
  ...
  const handleGeneratePDF = async (ticketData: any) => {
    ...
    // Función para resolver URLs a base64 usando el proxy
    const resolveImageUrl = async (url: string): Promise<string> => {
      ...
      try {
        ...
        console.warn(`Error resolviendo imagen ${url}:`, error)
        return ''
      }
    }
    ...
    // Extraer ítems
    let items = []
    if (Array.isArray(ticketData.products)) {
      items = ticketData.products
    } else if (ticketData.products && typeof ticketData.products === 'object') {
      items = Object.values(ticketData.products).flat()
    }
    ...
    const props = {
      ...
      contact: {
        label: 'Order simulation issued for:',
        name: ticketData.client?.name || 'Clark Kent, S.L.',
        address: ticketData.client?.address || 'Main Street 123, 28001 Madrid, Spain',
        phone: ticketData.client?.phone ? `Phone: ${ticketData.client.phone}` : 'Phone: +34 911 222 333',
        email: ticketData.client?.email || 'contact@testcompany.com',
        otherInfo: 'www.testcompany.com'
      },
      invoice: {
        label: 'Order simulation #: ',
        num: orderNumber,
        invDate: `Payment date: ${currentDate}`,
        invGenDate: `Order simulation date: ${currentDate}`,
        headerBorder: false,
        ...
        additionalRows: [
          {
            col1: 'Total:',
            col2: formatCurrency(Number(ticketData.totalPrice) || 0),
            col3: 'EUR',
            style: { fontSize: 14 }
          }
        ],
        invDescLabel: 'Order simulation note:',
        invDesc:
          'This simulation has been generated by AI. This simulation is not valid. This is part of a proof of concept.',
        resolveImageUrl // Pasar la función para resolver imágenes
      },
      footer: { text: 'This document is valid without signature or stamp.' },
      pageEnable: true,
      pageLabel: 'Page '
    }

    // Generar el PDF
    await jsPDFInvoiceTemplate(props)
  }
